...
class ScanIndex:
    ...
    # Parallel to local_files/remote_files: {key: {path: [line_numbers]}}.
    # Used by the web UI to show WHERE in each file the key matched.
    # Older entries from the cache may not have this field — handle gracefully.
...
def save_scan_cache(index: ScanIndex, keys: set[str], path: Path) -> None:
    try:
        payload = {
            ...
            "local_files_lines": index.local_files_lines,
            "remote_files_lines": index.remote_files_lines,
...
def scan_files_for_keys(keys: set[str], search_dirs: list, search_exts: set, skip_dirs: set, env_path: Optional[Path] = None, follow_symlinks: bool = False) -> tuple[dict[str, list[str]], dict[str, dict[str, list[int]]]]:
    """Return ({key: [paths]}, {key: {path: [line_numbers]}}) for files containing any key.

    The line-number index is parallel to the path index and lets the UI show
    WHERE in each file the key matched. Empty list means the key was matched
    but the exact line wasn't recorded (e.g. for very large files where we
    fell back to whole-file scan).
    """
    ...
    # Compile one re.Pattern per key ONCE; reuse for every file. Previously each
    # (file, key) pair called _key_matches → re.compile, which dominated scan time.
    ...
    for base in search_dirs:
        ...
        for root, dirnames, filenames in os.walk(bp, followlinks=follow_symlinks):
            ...
            for fn in filenames:
                ...
                checked += 1
                sp = str(fp)
                # Per-line scan so we can record which lines matched.
...
def scan_remote_files_for_keys(host: str, user: str, search_dirs: list, search_exts: set, skip_dirs: set, keys: set[str], key_path: Optional[Path] = None) -> tuple[dict[str, list[str]], dict[str, dict[str, list[int]]]]:
    """Returns ({key: [paths]}, {key: {path: [line_numbers]}}) for the remote host."""
    active = [k for k in keys if k]
    empty: tuple[dict[str, list[str]], dict[str, dict[str, list[int]]]] = ({}, {})
    if not active:
        return empty
    ...
    for root, dirs, files in os.walk(base):
        ...
        for fn in files:
            ...
            try:
                ...
                continue
            s = str(fp)
            file_lines = {k: [] for k in KEYS}
            for lineno, line in enumerate(text.splitlines(), 1):
                for k in KEYS:
                    if PATS[k].search(line):
                        file_lines[k].append(lineno)
            for k in KEYS:
                if file_lines[k] and s not in seen[k]:
                    seen[k].add(s)
                    index[k].append(s)
                    lines_index[k][s] = file_lines[k]
print(json.dumps({"index": index, "lines": lines_index}))
"""
    stdin_data = json.dumps({"keys": active, "exts": list(search_exts), "skip": list(skip_dirs), "dirs": list(search_dirs), "max_depth": 10, "boundary": _BOUNDARY_ESCAPED})
    ...
    if rc != 0:
        ...
        return sentinel, {k: {} for k in active}
    try:
        data = json.loads(out)
        return data.get("index", {}), data.get("lines", {})
...
"""
    stdin_data = json.dumps({"keys": active, "db_refs": [list(r) for r in db_refs], "boundary": _BOUNDARY_ESCAPED})
    rc, out, err = _ssh(host, user, f"python3 -c {__import__('shlex').quote(py)}",
...
def build_scan_index(
    ...
    # Fall back to CACHE_MAX_AGE_HOURS env var when caller doesn't supply one.
    ...
    local_files, local_files_lines = scan_files_for_keys(old_keys, search_dirs, search_exts, skip_dirs, env_path=env_path, follow_symlinks=follow_symlinks)
    local_dbs = scan_dbs_for_keys(key_db_refs) if key_db_refs else {k: [] for k in old_keys}

    remote_files: dict[str, dict[str, list[str]]] = {}
    remote_dbs: dict[str, dict[str, list[str]]] = {}
    remote_files_lines: dict[str, dict[str, dict[str, list[int]]]] = {}
        ...
        for rh in remote_hosts:
            ...
            try:
                ...
                with _Spinner(f"Scanning {label} files ({rh['host']})"):
                    rf, rfl = scan_remote_files_for_keys(rh["host"], rh["user"], rh["search_dirs"], search_exts, skip_dirs, old_keys, key_path=rh_key_path)
                ...
                remote_files_lines[label] = {k: {} for k in old_keys}
    else:
        print("  Skipping remote hosts.")

    index = ScanIndex(local_files, local_dbs, remote_files, remote_dbs,
                      local_files_lines=local_files_lines,
                      remote_files_lines=remote_files_lines,
